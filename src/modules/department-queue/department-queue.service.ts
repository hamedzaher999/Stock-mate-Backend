...
export class DepartmentQueueService {
  ...
  async create(dto: CreateQueueEntryDto, addedById: string) {
    ...
    const department = await this.departmentQueueRepository.findDepartmentType(
      dto.departmentId,
    );
    if (!department)
  ...
  async lock(id: string, doctorId: string) {
    ...
    const requesterContext =
      await this.departmentQueueRepository.findRequestingUserContext(doctorId);
    if (
  ...
  async remove(id: string, dto: RemoveQueueEntryDto, requestingUserId: string) {
    const entry = await this.findById(id, requestingUserId);
