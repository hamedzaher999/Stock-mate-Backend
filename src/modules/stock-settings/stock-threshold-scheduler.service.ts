...
export class StockThresholdSchedulerService {
    ...
    async handleCron() {
        ...
        this.logger.log(
            `Stock threshold check complete: ${result.breaches} breach(es) notified, ${result.alreadyNotified} already flagged today, ${result.skippedNoManager} skipped (no manager assigned).`,
        );
    }
    ...
    async runCheck(asOf: Date = new Date()): Promise<{
        ...
        for (const setting of settings) {
            ...
            if (isBelowMin) {
                const dedupeKey = `${key}:minimum:${todayKey}`;
                const flagged =
                    await this.notificationsService.wasAlreadyNotified(
                        NOTIFICATION_TYPES.STOCK_BELOW_MINIMUM,
                        'dedupeKey',
                        dedupeKey,
                    );
                if (flagged) {
            ...
            if (isAboveMax) {
                const dedupeKey = `${key}:maximum:${todayKey}`;
                const flagged =
                    await this.notificationsService.wasAlreadyNotified(
                        NOTIFICATION_TYPES.STOCK_ABOVE_MAXIMUM,
                        'dedupeKey',
                        dedupeKey,
                    );
                if (flagged) {
        ...
        return { breaches, alreadyNotified, skippedNoManager };
    }
}
