...
export class StockCountsService {
  constructor(private readonly stockCountsRepository: StockCountsRepository) {}
  ...
  async addItem(
    ...
    const variant = await this.stockCountsRepository.findVariant(dto.variantId);
  ...
  async complete(sessionId: string, requestingUserId: string) {
    ...
    const itemCount = await this.stockCountsRepository.countItems(sessionId);
    if (itemCount === 0)
      throw new BadRequestException(
