...
export class RefillRequestsController {
  constructor(private readonly refillRequestsService: RefillRequestsService) {}
  ...
  async findOne(
    ...
    const data = await this.refillRequestsService.findByIdForUser(id, user.sub);
    return { message: 'Success', data };
  }
  ...
  async submit(@Param('id') id: string) {
    ...
    return { message: 'Refill request submitted for hospital approval.', data };
  }
  ...
  async hospitalApprove(
    ...
    const data = await this.refillRequestsService.hospitalApprove(id, user.sub);
  ...
  @Post(':id/prepare')
  @RequirePermissions(PERMISSIONS.PREPARE_DEPARTMENT_REFILL)
  async prepare(@Param('id') id: string, @Body() dto: PrepareRefillRequestDto) {
