...
export class RefillRequestsService {
  ...
  async list(
    ...
    const departmentScope = await this.resolveDepartmentScope(requestingUserId);
  ...
  async update(id: string, dto: UpdateRefillRequestDto) {
    ...
    if (request.status !== 'draft')
      throw new ConflictException('Only draft refill requests can be edited.');
    ...
    return this.refillRequestsRepository.replaceItems(id, dto.notes, dto.items);
  }
  ...
  async prepare(id: string, dto: PrepareRefillRequestDto) {
    ...
    for (const prepared of dto.items) {
      const item = request.items.find((i) => i.id === prepared.refillItemId);
      if (item && prepared.preparedQuantity > Number(item.requestedQuantity)) {
        throw new BadRequestException(
    ...
    return this.refillRequestsRepository.setPreparedQuantities(id, dto.items);
  }
