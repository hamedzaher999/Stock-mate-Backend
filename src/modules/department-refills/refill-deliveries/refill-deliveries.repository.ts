...
export class RefillDeliveriesRepository {
  ...
  createDelivery(params: {
    ...
    notes?: string;
    lines: { refillItemId: string; batchId: string; shippedQuantity: number }[];
  }) {
  ...
  confirmDelivery(params: {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      for (const refillItemId of affectedItemIds) {
        ...
        const totalReceived = await tx.departmentRefillDeliveryItem.aggregate({
          where: { refillItemId, receivedQuantity: { not: null } },
          _sum: { receivedQuantity: true },
        });
        ...
        await tx.departmentRefillItem.update({
          ...
          data: {
            deliveredQuantity,
            quantityDiscrepancy:
              Number(item.preparedQuantity ?? 0) - deliveredQuantity,
          },
        });
        ...
        if (params.departmentType !== 'pharmacy') {
          ...
          await tx.departmentInventory.upsert({
            ...
            update: {
              currentQuantity: { increment: receivedInThisConfirmation },
      ...
      const fullyDelivered = allItems.every(
        (i) =>
          ...
          Number(i.deliveredQuantity ?? 0) >= Number(i.preparedQuantity),
      );
