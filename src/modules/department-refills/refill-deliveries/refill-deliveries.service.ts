...
export class RefillDeliveriesService {
  ...
  async create(dto: CreateDeliveryDto, deliveredById: string) {
    ...
    for (const inputItem of dto.items) {
      ...
      const batchStock = await this.refillDeliveriesRepository.findBatchStock(
        inputItem.batchId,
        warehouse.id,
      );
      if (!batchStock)
  ...
  async confirm(
    ...
    await this.assertDepartmentScope(requestingUserId, request.departmentId);
    ...
    for (const confirmedItem of dto.items) {
      ...
      if (
        confirmedItem.receivedQuantity > Number(deliveryItem.shippedQuantity)
      ) {
        throw new BadRequestException(
