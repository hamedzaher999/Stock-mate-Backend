...
export class PurchaseReceivingService {
  ...
  async list(dto: ListPurchaseReceiptsDto): Promise<PaginatedResult<unknown>> {
    const page = dto.page ?? 1;
    const limit = dto.limit ?? 20;

    const { items, total } = await this.purchaseReceivingRepository.findMany({
      skip: (page - 1) * limit,
      take: limit,
  ...
  async create(dto: CreatePurchaseReceiptDto, receivedById: string) {
    const order = await this.purchaseReceivingRepository.findOrderForReceiving(
      dto.purchaseOrderId,
    );
    if (!order) throw new BadRequestException('Purchase order does not exist.');
    ...
    const lines = dto.items.map((inputItem) => {
      ...
      const remaining =
        Number(orderItem.orderedQuantity) - Number(orderItem.receivedQuantity);
